...
- 캘린더 상단에는 `매일 오전 6시 갱신` 및 `일정 변동 가능` 안내 문구가 있습니다.
...
- 현재 수집 파이프라인은 `OpenDART 요약 + OpenDART 원문 증권신고서 + KIND 신규상장기업현황 + KIND 상세` 조합입니다.
...
- 상세 페이지는 `상장일 시초가`, `공모가 대비 수익률`, `일반청약 경쟁률`, `유통가능물량`, `IR/수요예측 일정`까지 표시합니다.
