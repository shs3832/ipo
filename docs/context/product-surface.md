...
- 종목 개요는 `이번 주 일정`, `그다음 일정`, `지난 종목` 섹션으로 나뉘고, `지난 종목`은 기본 접힘
- 종목 개요의 과거/현재 분류는 상장일이 있으면 상장일 기준이며, 청약이 끝났어도 상장일이 남은 종목은 `상장` 일정으로 계속 노출
- 단, 상장일이 청약 마감일보다 빠르거나 같은 비정상 값이면 유효 상장일로 보지 않고 청약 마감일 기준 fallback을 사용
- 시장구분/주관사/금액/최소청약주수 같은 공개 항목이 원본 placeholder 또는 missing 상태이면 `시장 미확인`, `주관사 미확인`, `데이터 미확보`처럼 명시적으로 표시
...
- 모바일/PWA에서 홈 최상단을 아래로 당기면 단순 페이지 새로고침만 수행하며, 공개 사용자 제스처로 `daily-sync`나 원천 데이터 강제 수집을 실행하지 않음
...
- 캘린더 일정은 `청약마감`, `환불`, `상장` 이벤트별 날짜 기준으로 표시
- 상장 이벤트는 청약 마감일보다 뒤인 유효 상장일만 표시하고, 검증 실패한 상장일은 캘린더 이벤트에서 제외

## Public Detail Page

- 상단 quick facts 우선
- 현재 핵심 항목:
  - `확정 공모가` 또는 확정 전 `희망 공모가`
  - `최소청약금액` 또는 희망밴드 기준 `예상 최소청약금액`
  - `환불일`
  - `상장예정일`
  - `유통가능물량`
  - `주관사`
- `offerPrice`는 확정 공모가로만 사용하고, 확정 전에는 `priceBandLow` / `priceBandHigh`로 희망 공모가를 표시
- 환불일/상장예정일/시장구분/주관사 등은 raw `-` 대신 `미확보`, `확인 필요`, `해당 없음` 상태를 명시
- 상장예정일이 청약 마감일 이전/당일이면 `상장일 확인 필요`로 표시하고, 상장 완료 판단에는 사용하지 않음
- 이벤트 타임라인은 제거됨
...
- 확정 공모가가 없더라도 희망밴드, 환불일, 주관사가 있으면 `일부 미확인` 상태로 알림을 보낼 수 있음
- 희망밴드 기준 금액은 알림 본문에서 `확정 전` / `희망밴드 기준` 문구를 함께 표시
- 생성되는 알림 링크는 `APP_BASE_URL` 기준이며 production에서는 `https:`만 허용
- 채널 preference 기반은 추가됐으며, 운영 기본값은 이메일 on / 앱푸시 off
- `/admin/recipients`에서 이메일 채널 on/off와 앱푸시 구독 저장/해제/테스트 발송을 조작할 수 있음
- 앱푸시는 Web Push 기반이며, 클릭 시 알림 payload의 same-origin 상세 경로 또는 기본 경로로 이동
