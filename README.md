...
- 종목 상세 페이지는 `지금 판단용` 핵심 정보, 분석 요약, 일정, 상세 데이터 순으로 우선순위 배치
...
- 현재는 핵심 근거가 부족한 종목에 대해 숫자 점수를 아예 숨기고 `평가 보류`로 표시합니다.
- 점수는 `총 4개 이상 지표`, `수급 2개 이상`, `재무 1개 이상`이 확보된 경우에만 노출됩니다.
- 점수가 보일 때도 `참고용 점수`와 반영 지표 요약을 함께 보여줍니다.
