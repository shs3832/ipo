...
- `OpenDART + KIND` 기반 공모주 일정/기초 재무/시세 데이터 수집

핵심 목표는 다음 3가지입니다.

- 공모주 일정을 캘린더에서 빠르게 확인
- 청약 마감 당일 오전 10시에 분석 메일 발송
- 공시 기반 체크 포인트와 알림으로 빠른 판단 지원
...
- 캘린더에서 오늘 날짜는 `오늘` badge와 강조 스타일로 표시
- 홈 화면 상단 히어로 + 요약 카드 + 월간 캘린더
- 캘린더 아래 `종목 개요` 카드 목록
- 현재 캘린더는 평일 기준으로만 보이며 토요일/일요일 열은 숨김 처리됨
- 모바일에서는 캘린더를 숨기고 `종목 개요` 중심으로 노출
- 종목 상세 페이지는 `지금 판단용` 핵심 정보, 공시 기반 체크 포인트, 일정, 상세 데이터 순으로 우선순위 배치
- 상세 페이지 상단에는 `확정 공모가`, `최소청약금액`, `환불일`, `상장예정일`, `유통가능물량`, `주관사`를 먼저 노출
- 상세 페이지는 `데이터 상태`를 함께 보여줘 핵심 일정/공모가 검증 여부를 바로 확인할 수 있음
- 공개 상세 페이지의 이벤트 타임라인은 제거하고 핵심 판단 정보 중심으로 정리
- 공개 화면과 메일에서는 현재 정량 점수를 비공개로 두고 공시 기반 안내를 우선 노출
...
- `OpenDART` 요약값만 부족한 항목은 증권신고서 `document.xml` 원문을 추가 파싱
- `listingDate`는 `KIND 공모일정(상장)`을 우선 참고하고 `KIND 신규상장기업현황`으로 추가 보강
- `확정 공모가`, `일반청약 경쟁률`, `IR 일정`, `수요예측 일정`, `유통가능주식수/비율`은 `KIND 상세`로 보강
- 상장일 당일 `시초가`와 `공모가 대비 수익률`은 `KIND 시세`로 보강
- 둘 다 없으면 빈 fallback 상태로 동작
- fallback 상태에서는 더미 공모주를 생성하지 않음
- 현재 OpenDART는 `현재월 + 다음월 청약 일정`을 캘린더 표시 대상으로 수집
- 내부적으로는 `두 달 전 시작 ~ 현재달 말` 공시를 참고해 다음달 일정까지 보강
- OpenDART `estkRs` 일반사항의 `배정기준일(asstd)`이 잡히는 건은 현재 임시 기준상 `실권주/배정형 비IPO`로 보고 캘린더에서 제외
- 날짜/월 경계 계산은 `Asia/Seoul` 기준 helper로 통일되어 있음
- `daily-sync` 이후 현재 표시 범위에서 소스에 없는 종목은 마지막 소스 확인 시각 기준 `2일` 유예 후 `WITHDRAWN`으로 정리됨
- 다만 위 `asstd` 기준으로 실권주/배정형 비IPO로 명시 판정된 건은 유예 없이 즉시 `WITHDRAWN` 처리됨
- 캘린더에는 `매일 오전 6시` 기본 갱신과 `상장일 시초가 10:10 / 10:30 추가 확인` 안내 문구를 노출
- `KIND 공모일정(상장)` + `KIND 상세`가 모두 연결되면 OpenDART에 없는 상장예정 종목도 신규 레코드로 생성 가능
- 다만 `KIND 공모일정`에만 있고 세부 연결이 안 되는 일정은 아직 완전한 KIND-first ingest가 아니다

현재 수집 조합은 아래 순서로 동작합니다.

1. `OpenDART estkRs` 요약값
2. `OpenDART` 증권신고서 `document.xml` 원문 zip 파싱
3. `KIND 공모일정(상장)`
4. `KIND 신규상장기업현황`
5. `KIND 상세(회사개요/공모정보)`
6. `KIND 시세(상장일 10:10 / 10:30 시초가 확인)`
...
- `prepare-closing-alerts`: 마감 30분 전 리마인더 payload 생성
- `dispatch-closing-alerts`: 마감 30분 전 리마인더 발송
...
- `prepare-daily-alerts`, `prepare-closing-alerts`는 최근 `90분` 내 성공한 `daily-sync`가 없으면 먼저 강제 재동기화를 시도
- `확정 공모가`, `환불일`, `주관사` 중 하나라도 비면 자동 알림 발송을 보류하고 운영 로그에 남김
- `상장 예정일`이 비어도 자동 발송은 유지하되 `데이터 상태: 일부 미확인`으로 표시
- 핵심 정보는 있지만 일부 보조 항목이 비면 `데이터 상태: 일부 미확인`으로 표시한 채 알림을 보냄
- 메일 payload에는 `데이터 상태` 섹션이 포함되어 어떤 항목이 확인됐는지 / 추가 확인 중인지 함께 안내함
...
점수는 [src/lib/analysis.ts](/Users/shs/Desktop/Study/ipo/src/lib/analysis.ts)에서 규칙 기반으로 내부 계산합니다.
...
- 그래서 점수는 아직 `완성형 투자 판단 점수`가 아니라 `내부 휴리스틱`에 가깝습니다.
- 현재는 공개 화면과 메일에서 정량 점수를 비공개로 유지하고, 공시 기반 체크 포인트만 제공합니다.
- 점수 로직과 `scoreDisplay` 메타데이터는 추후 재공개 판단과 내부 검증을 위해 유지합니다.
- 점수 재공개 전제는 추가 데이터 소스 확보, 근거 품질 검증, 라이브 결과 확인입니다.
...
- `유통가능물량`은 저장 시점에 이미 percent 값(`14`, `23.5`)으로 정규화됩니다.
- UI에서 다시 `formatPercent`처럼 100배 처리하면 안 되고, 그대로 `%`만 붙여야 합니다.

주의:

- KIND 상장 데이터도 시점에 따라 아직 공개되지 않았거나 지연될 수 있습니다.
- 일정 정보는 증권사/거래소 공고에 따라 바뀔 수 있으므로 최종 확인이 필요합니다.
- 현재 실권주 제외는 `asstd` 1개 신호를 쓰는 임시 기준이며, 향후 KIND-first 캘린더 및 공시 본문 분류로 보강 예정입니다.
...
- `01:10 UTC` -> `10:10 KST` `daily-sync` (상장일 시초가 추가 확인)
- `01:30 UTC` -> `10:30 KST` `daily-sync` (상장일 시초가 재확인)
- `00:00 UTC` -> `09:00 KST` `prepare-daily-alerts`
- `01:00 UTC` -> `10:00 KST` `dispatch-alerts`
- `06:25 UTC` -> `15:25 KST` `prepare-closing-alerts`
- `06:30 UTC` -> `15:30 KST` `dispatch-closing-alerts`
